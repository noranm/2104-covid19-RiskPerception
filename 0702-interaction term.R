...
# Political
risk_base$Political <- factor(risk_base$XD2, levels=c(1, 3, 2, 9999), 
                          labels=c("Conservative", "Liberal", "Neutral", "No opinion") )
...
fit1 <- glm(Anxiety~ Trust + Sex + Age + Job + household + Area + Political + Trust * Sex + phaseName,
            data=risk_base_2, weights=risk_base_2$WT2, family="binomial")
...
fit2 <- glm(Likelihood ~ Trust + Sex + Age + Job + household + Area + Political + Trust * Sex + phaseName,
            data=risk_base_2, weights=risk_base_2$WT2, family="binomial")
...
file_format <- file('./[210702]Overall_intSex.csv',encoding="euc-kr")
...
for (i in phase){
  ...
  fit <- glm(Anxiety~ Trust + Sex + Age + Job + household + Area + Political + Trust * Sex,
             data=each.Surv, weights=each.Surv$WT2, family="binomial")
  ...
  final <- rbind(final, fit.var)
}

file_format <- file('./[210702]eachPhase_Affective_intSEX.csv', encoding="euc-kr")
...
for (i in phase){
  ...
  fit <- glm(Likelihood~ Trust + Sex + Age + Job + household + Area + Political + Trust * Sex,
                data=each.Surv,  weights=each.Surv$WT2, family="binomial")
  fit.var <- data.frame("variable" = names(fit$coefficients),
                        "exp" = exp(fit$coefficients),
                        "lower" = exp(summary(fit)$coef[,1] - 1.96 * summary(fit)$coef[,2]), # CI
...
file_format <- file('./[210702]eachPhase_Cognitive_intSEX.csv', encoding="euc-kr")
write.csv(final, file_format, row.names=FALSE)


# overall analysis - phase
fit1 <- glm(Anxiety~ Trust + Sex + Age + Job + household + Area + Political + Trust * Age + phaseName,
            data=risk_base_2, weights=risk_base_2$WT2, family="binomial")
...
fit2 <- glm(Likelihood ~ Trust + Sex + Age + Job + household + Area + Political + Trust * Age + phaseName,
            data=risk_base_2, weights=risk_base_2$WT2, family="binomial")
...
file_format <- file('./[210702]Overall_intAge.csv',encoding="euc-kr")
write.csv(overall.fit, file_format)
...
for (i in phase){
  ...
  fit <- glm(Anxiety~ Trust + Sex + Age + Job + household + Area + Political + Trust * Age,
             data=each.Surv, weights=each.Surv$WT2, family="binomial")
  fit.var <- data.frame("variable" = names(fit$coefficients),
                        "exp" = exp(fit$coefficients),
                        "lower" = exp(summary(fit)$coef[,1] - 1.96 * summary(fit)$coef[,2]), # CI
...
file_format <- file('./[210702]eachPhase_Affective_intAge.csv', encoding="euc-kr")
write.csv(final, file_format, row.names=FALSE)
...
for (i in phase){
  ...
  print(dim(each.Surv))
  each_id <- 1:nrow(each.Surv)
  fit <- glm(Likelihood~ Trust + Sex + Age + Job + household + Area + Political + Trust * Age,
             data=each.Surv,  weights=each.Surv$WT2, family="binomial")
  fit.var <- data.frame("variable" = names(fit$coefficients),
                        "exp" = exp(fit$coefficients),
                        "lower" = exp(summary(fit)$coef[,1] - 1.96 * summary(fit)$coef[,2]), # CI
...
file_format <- file('./[210702]eachPhase_Cognitive_intAge.csv', encoding="euc-kr")
write.csv(final, file_format, row.names=FALSE)
