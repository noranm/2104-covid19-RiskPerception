...
confirmed$WEEK2 = cut(as.Date(confirmed$Date, "%m/%d/%y")+7, breaks="week")
confirmed_week <- confirmed %>% group_by(WEEK2) %>% summarise(weekly_cases = sum(Daily_confirmed)) %>% data.frame()
confirmed_week$WEEK2 <- as.Date(as.character(confirmed_week$WEEK2))
head(confirmed_week)
...
survey <- merge(survey, confirmed_week, by='WEEK2', all.x=TRUE)
survey$logN <- log10(survey$weekly_cases)

risk <- merge(risk, survey, all.x=TRUE, by="WEEK2")
head(risk)

# interested in : BK5, BK6
risk_base <- risk %>% select(DATE, ARA, AGE, SEX, BK5, BK6, SAGE, WT2, JOB, XD2, XD3, BQ1, BQ3, survName, phaseName, logN)
...
risk_base$TrustNoOp <- as.numeric(risk_base$BQ1==9999)
table(risk_base$TrustApproval)
table(risk_base$TrustNeutral)
table(risk_base$TrustNoOp)
...
risk_base$PoliticalLiberal <- as.numeric(risk_base$XD2 == 3)
risk_base$PoliticalNeutral <- as.numeric(risk_base$XD2 == 2)
risk_base$PoliticalNoOp <- as.numeric(risk_base$XD2 == 9999)

table(risk_base$PoliticalLiberal)
table(risk_base$PoliticalNeutral)
table(risk_base$PoliticalNoOp)
...
fit1 <- glm(Anxiety~ TrustApproval + TrustNeutral + TrustNoOp + 
                 SEX + AGE3039 + AGE4049 + AGE5059 + AGE60 + 
                 FarmForFish + SelfEmployed + Blue + White + HomeMakerStudent + 
                 Middle + LowerMiddle + 
                 Chung + Yeong + Ho +AreaETC +
                 PoliticalLiberal + PoliticalNeutral + PoliticalNoOp + 
                 Phase2 + Phase3 + Phase4 + Phase5 + logN, data=risk_base_2, weights=risk_base_2$WT2, family="binomial")
...
fit2 <- glm(Likelihood ~ TrustApproval + TrustNeutral + TrustNoOp + 
                 SEX + AGE3039 + AGE4049 + AGE5059 + AGE60 + 
                 FarmForFish + SelfEmployed + Blue + White + HomeMakerStudent + 
                 Middle + LowerMiddle + 
                 Chung + Yeong + Ho +AreaETC +
...
file_format <- file('./[210810]Overall-confirmed.csv',encoding="euc-kr")
...
for (i in phase){
  ...
  fit <- glm(Anxiety~ TrustApproval + TrustNeutral + TrustNoOp + 
               SEX + AGE3039 + AGE4049 + AGE5059 + AGE60 + 
               FarmForFish + SelfEmployed + Blue + White + HomeMakerStudent + 
               Middle + LowerMiddle + 
               Chung + Yeong + Ho +AreaETC +
               PoliticalLiberal + PoliticalNeutral + PoliticalNoOp + logN,, data=each.Surv, weights=each.Surv$WT2, family="binomial")
...
file_format <- file('./[210810]eachPhase_Affective_confirmed.csv',encoding="UTF-8")
write.csv(final1, file_format, row.names=FALSE)
...
for (i in phase){
  ...
  fit <- glm(Likelihood~ TrustApproval + TrustNeutral + TrustNoOp + 
               SEX + AGE3039 + AGE4049 + AGE5059 + AGE60 + 
               FarmForFish + SelfEmployed + Blue + White + HomeMakerStudent + 
               Middle + LowerMiddle + 
               Chung + Yeong + Ho +AreaETC +
...
file_format <- file('./[210810]eachPhase_Cognitive_confirmed.csv',encoding="UTF-8")
write.csv(final2, file_format, row.names=FALSE)
...
# TRUST plot
table(final1$variable)
Results_trust1 <- final1[final1$variable %in% c("TrustApproval", "TrustNeutral", "TrustNoOp"),]
...
Results_trust2 <- final2[final2$variable %in% c("TrustApproval", "TrustNeutral", "TrustNoOp"),]
...
table(final1$variable)
Results_party1 <- final1[final1$variable %in% c("PoliticalLiberal", "PoliticalNeutral", "PoliticalNoOp"),]
...
Results_party2 <- final2[final2$variable %in% c("PoliticalLiberal", "PoliticalNeutral", "PoliticalNoOp"),]
